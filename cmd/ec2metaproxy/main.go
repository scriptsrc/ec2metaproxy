...
type RoleArnValue ec2metaproxy.RoleArn

func (t *RoleArnValue) Set(value string) error {
	if len(value) > 0 {
		arn, err := ec2metaproxy.NewRoleArn(value)
		*(*ec2metaproxy.RoleArn)(t) = arn
		return err
	}
...
func RoleArnOpt(s kingpin.Settings) (target *ec2metaproxy.RoleArn) {
	target = new(ec2metaproxy.RoleArn)
